...
int main(){ 
 ...
 for (int i=1;i<=m;i++) 
 if (init(q[i].k1,x[A[i].where].s,x[A[i].where].t)){ 
 ...
 point k1=A[i].t; long long rem=q[i].t-A[i].d; int where=A[i].where;
 for (int j=53;j>=0;j--) 
...
int main(){ 
 ...
 return 0; 
        ...
        a_tokens = tokenize(a)
        b_tokens = tokenize(b)
        # a_tokens = a_tokens[1800:]
        # b_tokens = b_tokens[1800:]
        print('after tokenize: {}, {}'.format(len(a_tokens), len(b_tokens)))
        ...
        distance, _ = levenshtenin_distance(a_tokens, b_tokens, equal_fn=generate_equal_fn(get_token_value))
